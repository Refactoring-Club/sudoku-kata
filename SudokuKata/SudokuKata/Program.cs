...
namespace SudokuKata
{
    class Program
    {
        static void Play()
        {
            ...
            // Prepare empty board
            var _board = Board.CreateEmptyBoard();
            var board = _board.AsArray();
            var state = _board.State.Peek();
            var finalState = _board.SolvedState;
            ...
            while (changeMade)
            {
                ...
                if (!changeMade)
                {
                    ...
                    while (candidateIndex1.Any())
                    {
                        ...
                        var (stateStack, rowIndexStack, colIndexStack, usedDigitsStack, lastDigitStack) = Stacktory.StackFactory();
